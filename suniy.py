...
# === Klientlar ===
client = OpenAI(api_key=OPENAI_API_KEY)  # proxy olib tashlandi
...
def start(message):
    ...
    if user_id not in users:
        users[user_id] = {"username": username, "is_vip": False, "vip_expiry": None,
...
def admin_panel(message):
    ...
    markup.add(types.InlineKeyboardButton("➕ VIP ulash", callback_data="add_vip"),
...
def process_vip_username(message, action):
    ...
    for uid, data in users.items():
        if data["username"] == username:
            ...
            break
    if not user_id:
        return bot.reply_to(message, "❌ Bunday foydalanuvchi topilmadi.")
    if action == "add_vip":
        ...
        try: bot.send_message(user_id, "🎉 Sizga 30 kunlik Premium hisob ulandi!\nEndi cheksiz savollar bera olasiz.")
        except: pass
    else:
        ...
        try: bot.send_message(user_id, "⚠️ Sizning VIP hisobingiz o‘chirildi.")
        except: pass
...
def handle_message(message):
    ...
    text = message.text
    if user_id not in users:
        users[user_id] = {"username": message.from_user.username or "Noma'lum",
                          "is_vip": False, "vip_expiry": None, "daily_uses": 0, "last_use_date": None}
    user = users[user_id]
    ...
    try:
        ...
        # Yangi SDK bilan moslash
        answer = getattr(response.choices[0].message, "content", None)
        if not answer:  # fallback
            answer = response.choices[0].content[0].text
        bot.reply_to(message, answer)
        if not user["is_vip"]: user["daily_uses"] += 1
